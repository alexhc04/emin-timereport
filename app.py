...
@app.route('/health')
def health():
    """Diagnóstico completo — visita /health para ver qué falla"""
    info = {
        'status': 'ok',
        'python': __import__('sys').version,
        'database_url_definida': bool(os.environ.get('DATABASE_URL')),
        'database_url_preview': (os.environ.get('DATABASE_URL', 'NO DEFINIDA')[:40] + '...') if os.environ.get('DATABASE_URL') else 'NO DEFINIDA',
    }
    try:
        ...
        info['database'] = 'ERROR'
        info['error_detalle'] = str(e)
    return jsonify(info)


# ─────────────────────────────────────────────
# INICIALIZACIÓN AUTOMÁTICA (gunicorn + python app.py)
# Se envuelve en try/except para que un fallo de BD
# no impida que gunicorn arranque y muestre el error
# ─────────────────────────────────────────────

try:
    init_db()
except Exception as e:
    print(f"⚠️  init_db() falló al arrancar: {e}")
    print("La app seguirá funcionando; visita /health para diagnóstico")


# ─────────────────────────────────────────────
# PUNTO DE ENTRADA (solo desarrollo local)
# ─────────────────────────────────────────────
